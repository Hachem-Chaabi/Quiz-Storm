...
const create = async (data: { sessionId: string; userId: string }) => {
  return prisma.sessionParticipant.create({ data });
};

const edit = async (id: string, data: SessionParticipant) => {
...
const remove = async (id: string) => {
  return prisma.sessionParticipant.delete({ where: { id } });
};
