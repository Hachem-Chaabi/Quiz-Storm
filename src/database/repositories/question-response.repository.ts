...
type ConditionObj = {
  [K in keyof QuestionResponse]?: QuestionResponse[K];
};
...
const getOneByQuery = async (where: ConditionObj, select: object = {}) => {
  return prisma.questionResponse.findFirst({
    where,
    select: Object.keys(select).length ? select : undefined,
  });
};

const create = async (data: QuestionResponse) => {
  return prisma.questionResponse.create({ data });
};

const edit = async (id: string, data: QuestionResponse) => {
...
const remove = async (id: string) => {
  return prisma.questionResponse.delete({ where: { id } });
};
